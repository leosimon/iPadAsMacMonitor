...
3. **首次运行**时，macOS 会弹出安全提示：
   - 前往 **系统设置 → 隐私与安全性**，点击**"仍要打开"**
   - 随后授予**辅助功能权限**：前往 **系统设置 → 隐私与安全性 → 辅助功能**，启用该 app
4. **再次运行**，几秒后 iPad 即会作为副屏连接

> **为什么首次运行会被拦截？**
> 该 app 使用临时签名（Ad-hoc）发布，无需付费的 Apple 开发者账号。
> macOS 对所有未经苹果注册开发者签名的 app 首次运行都会提示确认，这是系统的安全机制。
> 只需授权一次，之后每次运行都不会再弹出任何提示。
